...
wait_done() {
  local seed="$1"
  local log="exp_report/fusion_norm_ablation/${stamp}/clothing_seed${seed}.log"
  while ! grep -q "stage2_clothing_rankres_allgate_seed${seed}_${stamp} best epoch" "${log}" 2>/dev/null; do
    date "+[watch-clothing] %F %T waiting clothing seed${seed}"
    sleep 120
  done
}
...
latest_ckpt() {
  local seed="$1"
  find "exp_report/clothing/stage2_clothing_rankres_allgate_seed${seed}_${stamp}/ckpt" \
...
eval_one() {
  ...
  local base_log="$2"
  local base_ckpt="$3"
  local fusion_log="exp_report/fusion_norm_ablation/${stamp}/clothing_seed${seed}.log"
  local fusion_ckpt
  fusion_ckpt="$(latest_ckpt "${seed}")"

  echo "[eval-clothing] seed${seed} ckpt=${fusion_ckpt}"
  CUDA_VISIBLE_DEVICES=3 .venv/bin/python tools/evaluate_late_score_fusion.py \
    ...
    > "${out}/clothing_seed${seed}_test.log" 2>&1
}
...
lines = [
    "# All-modal normalized gate clothing 2seed",
    "",
...
for seed in seeds:
    rows = parse(out / f"clothing_seed{seed}_test.log")
    br, bn = rows[0.0]
    fr, fn = rows[0.4]
    ar, an = rows[1.0]
...
(out / "clothing_summary.md").write_text("\n".join(lines) + "\n")
print(out / "clothing_summary.md")
PY
