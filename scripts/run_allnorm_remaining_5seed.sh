...
stamp="${3:-unified_allnorm_5seed_20260528}"
...
run_one() {
  ...
  local gate_reg="$4"
  local suffix="stage2_${dataset}_rankres_allnorm_seed${seed}_${stamp}"
  local outer_dir="exp_report/fusion_norm_ablation/${stamp}"

  mkdir -p "${outer_dir}"
  echo "[allnorm] dataset=${dataset} seed=${seed} gpu=${gpu} suffix=${suffix}"
