...
SUFFIX="${SUFFIX:-stage3_baby_joint_finetune_${EXP_MODE}}"
...
exec env CONFIG="${CONFIG}" \
  ...
  --beta_intra "${BETA_INTRA:-0.01}" \
  --beta_inter "${BETA_INTER:-0.01}" \
  --beta_itm "${BETA_ITM:-0.01}" \
  --beta_rec "${BETA_REC:-0.002}" \
  --beta_decode "${BETA_DECODE:-0.005}" \
  --save "${SAVE}" \
  --suffix "${SUFFIX}" \
  --device_id "${DEVICE_ID}" \
  "$@"
