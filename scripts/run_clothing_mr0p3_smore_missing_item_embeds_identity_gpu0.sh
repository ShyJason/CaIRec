...
GPU="${GPU:-0}"
RUN_TAG="${RUN_TAG:-clothing_mr0p3_smore_missing_item_embeds_identity_$(date +%Y%m%d_%H%M%S)}"
OUT_DIR="${OUT_DIR:-exp_report/clothing/smore_missing_input_item_embeds_identity/${RUN_TAG}}"
...
run_variant() {
  ...
  suffix="stage2_clothing_mr0p3_smore_missing_${variant}_item_embeds_identity_no_completion_no_ii_${RUN_TAG}"
  log_path="${LOG_DIR}/${variant}_item_embeds_identity.log"

  log "start ${variant} missing item_embeds identity; feature_dir=${feature_dir}"
  .venv/bin/python -u main.py \
    ...
    --topk "[10, 20, 30, 40, 50]" \
    > "${log_path}" 2>&1
  log "done ${variant} missing item_embeds identity; log=${log_path}"
}
