...
GPU="${GPU:-7}"
RUN_TAG="${RUN_TAG:-dynamic_conf_itemgraph_gpu${GPU}_$(date +%Y%m%d_%H%M%S)}"
OUT_DIR="${OUT_DIR:-exp_report/clothing/dynamic_conf_itemgraph_mr0p3/${RUN_TAG}}"
...
SUFFIX="stage2_clothing_mr0p3_dynamic_conf_${RUN_TAG}"
...
CMD=(
  ...
  --item_graph_kind fused_completed_dynamic_confidence
  --item_graph_topk 8
  --item_graph_norm rw
  ...
  --item_graph_rr_confidence_init 0.95
  --item_graph_ri_confidence_init 0.95
  --item_graph_ii_confidence_init 0.95
  --tensorboard 0
  --save 1
  --topk "[10, 20, 30, 40, 50]"
)
