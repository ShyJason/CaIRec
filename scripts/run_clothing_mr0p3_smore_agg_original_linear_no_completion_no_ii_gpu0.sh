...
GPU="${GPU:-0}"
RUN_TAG="${RUN_TAG:-clothing_mr0p3_smore_agg_original_linear_no_completion_no_ii_$(date +%Y%m%d_%H%M%S)}"
OUT_DIR="${OUT_DIR:-exp_report/clothing/smore_agg_original_linear_no_completion_no_ii/${RUN_TAG}}"
...
run_variant() {
  local name="$1"
  local feature_dir="$2"
  local suffix="stage2_clothing_mr0p3_smore_${name}_agg_original_linear_no_completion_no_ii_${RUN_TAG}"
