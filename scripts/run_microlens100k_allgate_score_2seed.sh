...
gpu="${GPU:-4}"
stamp="${STAMP:-microlens100k_allgate_score_2seed_$(date +%Y%m%d_%H%M%S)}"
seeds="${SEEDS:-12 123}"
...
imputer_ckpt_for_seed() {
  ...
  case "${seed}" in
    12)
      echo "exp_report/microlens100k/stage1_2_microlens100k_imputer_backprop_decoder_v2_mmrec_microlens100k_3modal_seed12_mbpr25_20260523_033613/ckpt/stage1_2_microlens100k_imputer_backprop_decoder_v2_mmrec_microlens100k_3modal_seed12_mbpr25_20260523_033613_imputer_backprop_50_epoch18.pth"
      ;;
    123)
      echo "exp_report/microlens100k/stage1_2_microlens100k_imputer_backprop_decoder_v2_mmrec_microlens100k_3modal_seed123_mbpr25_20260523_033613/ckpt/stage1_2_microlens100k_imputer_backprop_decoder_v2_mmrec_microlens100k_3modal_seed123_mbpr25_20260523_033613_imputer_backprop_50_epoch13.pth"
      ;;
    *)
...
run_stage2() {
  ...
  if [[ -f "${log_path}" ]] && grep -q "best epoch" "${log_path}"; then
    echo "[microlens100k-allgate] skip finished seed=${seed} mode=${mode}"
    return
  fi

  echo "[microlens100k-allgate] run seed=${seed} mode=${mode} gpu=${gpu} suffix=${suffix}"
  local gate_args=()
...
eval_seed() {
  ...
  echo "[microlens100k-allgate] eval seed=${seed}"
  CUDA_VISIBLE_DEVICES="${gpu}" .venv/bin/python tools/evaluate_late_score_fusion.py \
    ...
    > "${eval_dir}/microlens100k_seed${seed}_test.log" 2>&1
}
...
lines = [
    "# MicroLens-100k all-modal normalized gate 2seed",
    "",
