...
class ProMRL(MMGeneralModule):
    ...
    def __init__(self, config):
        ...
        self.W = {"v": self.W_v, "a": self.W_a, "t": self.W_t}
        self.mu = {"v": self.mu_v, "a": self.mu_a, "t": self.mu_t}
        self.sigma2 = {"v": self.sigma2_v, "a": self.sigma2_a, "t": self.sigma2_t}
        
        # load the features [temp feature]
        temp_feature = torch.load("feature_inference_results/adam/model2.pt", weights_only=True)
        for i in self.W:
    ...
    def forward_ret(self, batch, task, compute_loss=True):
        ...
        if compute_loss:
            ...
            if self.update:
                ...
                # Step 2: Update generative params (only for observed)
                # W_dict, mu_dict, sigma_dict = update_generative_params(
                #     obs_feats, m, V, modalities, self.d_beta * 2
                # )
                # for k in modalities: # EMA update generative params
                #     self.W[k].data = self.W[k].data * (1-self.ema_eta) + W_dict[k].data * self.ema_eta
                #     self.mu[k].data = self.mu[k].data * (1-self.ema_eta) + mu_dict[k].data * self.ema_eta
                #     self.sigma2[k].data = self.sigma2[k].data * (1-self.ema_eta) + sigma_dict[k].data * self.ema_eta
            ...
            missing_modalities = set(["v", "a","t"]) - set(modalities)
            ...
            for task in subtasks:
                assert task in ['tv','ta','tva', 'va']
                
                # targets = torch.linspace(rank * bs, rank * bs + bs - 1, bs, dtype=int).to(feat_cond.device)
                
                # if "raw_subtitles" in batch.keys():
                #     eigenvectors, eigenvalues = eigenvalue_computation_pmcl([feat_t,feat_v,feat_a,feat_s])
