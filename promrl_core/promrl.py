...
from .utils.impute import update_posterior, update_generative_params, compute_nll_loss
from torch.nn.modules.container import ParameterDict
import math

# Allow ParameterDict to be loaded safely on newer PyTorch versions.
if hasattr(torch.serialization, "add_safe_globals"):
    torch.serialization.add_safe_globals([ParameterDict])

class ProMRL(MMGeneralModule):
    """ PMRL pretraining """
    def __init__(self, config):
        ...
        self.lambda_itm = config.lambda_itm
        
        self.ema_eta = getattr(config, "ema_eta", 0.01)
        
        # generative parameters >>>
        if "beta_0.1" in config.log_name:
            self.d_beta = contra_dim // 10
            temp_feature = torch.load("feature_inference_results/adam/model_b0.1.pt", weights_only=True)
        elif "beta_0.2" in config.log_name:
            self.d_beta = contra_dim // 5
            temp_feature = torch.load("feature_inference_results/adam/model_b0.2.pt", weights_only=True)
        elif "beta_1" in config.log_name:
            self.d_beta = contra_dim // 1
            temp_feature = torch.load("feature_inference_results/adam/model_b1.pt", weights_only=True)
        else:
            self.d_beta = contra_dim // 2
            temp_feature = torch.load("feature_inference_results/adam/model.pt", weights_only=True)
            
        self.W_v = nn.Parameter(torch.randn(contra_dim, self.d_beta), requires_grad=False)
        self.W_a = nn.Parameter(torch.randn(contra_dim, self.d_beta), requires_grad=False)
        self.W_s = nn.Parameter(torch.randn(contra_dim, self.d_beta), requires_grad=False)
        self.W_t = nn.Parameter(torch.randn(contra_dim, self.d_beta), requires_grad=False)

        self.mu_v = nn.Parameter(torch.randn(contra_dim, self.d_beta), requires_grad=False)
        self.mu_a = nn.Parameter(torch.randn(contra_dim, self.d_beta), requires_grad=False)
        self.mu_s = nn.Parameter(torch.randn(contra_dim, self.d_beta), requires_grad=False)
        self.mu_t = nn.Parameter(torch.randn(contra_dim, self.d_beta), requires_grad=False)
        
        self.sigma2_v = nn.Parameter(torch.tensor(1.0), requires_grad=False)
        self.sigma2_a = nn.Parameter(torch.tensor(1.0), requires_grad=False)
        self.sigma2_s = nn.Parameter(torch.tensor(1.0), requires_grad=False)
        self.sigma2_t = nn.Parameter(torch.tensor(1.0), requires_grad=False)
        # <<< generative parameters
        
        self.W = {"v": self.W_v, "a": self.W_a, "s": self.W_s, "t": self.W_t}
        self.mu = {"v": self.mu_v, "a": self.mu_a, "s": self.mu_s, "t": self.mu_t}
        self.sigma2 = {"v": self.sigma2_v, "a": self.sigma2_a, "s": self.sigma2_s, "t": self.sigma2_t}
        
        # load the features [temp feature]
        
        for i in self.W:
            self.W[i].data = temp_feature["W"][i].data
            self.mu[i].data = temp_feature["mu"][i].data
            self.sigma2[i].data = temp_feature["log_sigma"][i].data # log_sigma to sigma2
            
        print("load the features [temp feature]")
        
        self.update = True # if  false, you may need to update the generative parameters manually
        self.feature_inference = config.feature_inference
        # if self.feature_inference:
        #     self.Z = None
        #     self.update = False
    ...
    def forward_ret(self, batch, task, compute_loss=True):
        ...
        if "raw_captions" in batch.keys() and isinstance(batch.raw_captions[0],list): #### test
        ...
        if compute_loss:
            ...
            # obs_feats = {"v": None, "a": None, "s": None, "t": None}
            obs_feats = {}
            modalities = []
            ...
            if self.update:
                ...
                with torch.no_grad():
                    m, V = update_posterior(obs_feats, 
                                            self.W, 
                                            self.mu, 
                                            self.sigma2, # log format for adam feature
                                            modalities, 
                                            self.d_beta)
                    
                rec_loss, rec_loss_dict = compute_nll_loss(obs_feats, m.detach(), self.W, self.mu, self.sigma2, modalities)
                loss_dict['rec_loss'] = rec_loss
            
            # Step 3: Impute missing
            missing_modalities = set(["v", "a", "s", "t"]) - set(modalities)
            for k in missing_modalities:
                if k=="t": feat_t = m @ self.W[k].T + self.mu[k].unsqueeze(0)
                elif k=="v": feat_v = m @ self.W[k].T + self.mu[k].unsqueeze(0)
                elif k=="a": feat_a = m @ self.W[k].T + self.mu[k].unsqueeze(0)
                elif k=="s": feat_s = m @ self.W[k].T + self.mu[k].unsqueeze(0)
                        
            for task in subtasks:
                assert task in ['tv','ta','tva','tvs','tvas', 'va']
                
                # remove the subtitles learning.
                eigenvectors, eigenvalues = eigenvalue_computation_pmcl([feat_t,feat_v,feat_a])
                eigenvalues = eigenvalues / self.tau1

                # Compute the loss for intra-modal alignment
                loss_intra_ = F.cross_entropy(eigenvalues, torch.zeros(eigenvalues.shape[0]).to(eigenvalues.device).long())
                ...
                targets = torch.linspace(rank * bs, rank * bs + bs - 1, bs, dtype=int).to(principal_eigenvector.device)
                
                loss_inter_ = F.cross_entropy((principal_eigenvector @ principal_eigenvector_all.T)/self.tau2, targets)
                ...
                if 't' in task:
                    ...
                    loss_itm.append(torch.tensor(0.0))

            loss_intra = sum(loss_intra)/len(loss_intra)
            loss_dict['loss_intra'] = loss_intra
            loss_inter = sum(loss_inter)/len(loss_inter)
            loss_dict['loss_inter'] = loss_inter
            ...
            if "a" in subtasks_str:
                feat_a = self.batch_get(batch,'feat_a')
                evaluation_dict['feat_a'] = feat_a 
            if "v" in subtasks_str:
                feat_v = self.batch_get(batch,'feat_v')
                evaluation_dict['feat_v'] = feat_v 
            if "s" in subtasks_str:
                feat_s = self.batch_get(batch,'feat_s')
