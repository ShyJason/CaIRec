...
SCOPE="${SCOPE:-${1:-both}}"
PHYSICAL_GPU="${PHYSICAL_GPU:-${2:-0}}"
SCALE="${SCALE:-50}"
CF_SCALE="${CF_SCALE:-raw}"
CHECK_ONLY="${CHECK_ONLY:-0}"
case "${SCOPE}" in
  graph|fusion|both) ;;
  *) echo "scope must be graph, fusion, or both" >&2; exit 2 ;;
esac
...
RUN_TAG="${RUN_TAG:-clothing_unified_mr0p5_seed2023_posterior_rel_${SCOPE}_scale${SCALE}_20260719}"
OUT_DIR="exp_report/clothing/posterior_reliability_unified/${RUN_TAG}"
...
echo "dataset=clothing protocol=unified_static mr=0.5 seed=2023 payload_seed=2023 scope=${SCOPE} scale=${SCALE} cf_scale=${CF_SCALE}"
echo "checkpoint=${IMPUTER_CKPT}"
if [[ "${CHECK_ONLY}" == "1" ]]; then
  echo "preflight passed; no training started"
  exit 0
fi
test ! -e "${LOG_FILE}" || { echo "refusing to overwrite: ${LOG_FILE}" >&2; exit 2; }
...
env \
  ...
  ./run_demo_itm.sh \
    ...
    --item_graph_kind modality_completed --item_graph_missing_scope train \
    --item_graph_feature_space shared \
    --item_graph_topk 10 --item_graph_norm rw \
    --item_graph_cf_weight 0.4 --item_graph_cf_scale "${CF_SCALE}" \
    ...
    --posterior_reliability_scope "${SCOPE}" \
    --posterior_reliability_scale "${SCALE}" \
